...
def get_config(config, mode='train', weight_path=None):
    ...
    with open(config, 'r') as stream:
        ...
        cfg['data']['load_weights_dir_1'] = weight_path
...
def cal_depth_error_del(depth_pred, depth_gt,mask,min_eval_depth, max_eval_depth,type="met"):
    # print("--------:++++++++++",depth_gt.size(),depth_pred.size(),mask.size())



    # depth_gt = depth_gt[indices]
    # depth_pred = depth_pred[indices]
    scale_val = torch.median(depth_gt[mask]) / torch.median(depth_pred[mask])
    ...
    all=mask.sum()
    if type=="met":
        abs_rel = torch.abs(depth_pred_metric - depth_gt) / depth_gt
        sq_rel = (depth_pred_metric - depth_gt) ** 2 / depth_gt
        rmse = (depth_pred_metric - depth_gt) ** 2
        rmse_log = (torch.log(depth_pred_metric) - torch.log(depth_gt)) ** 2
        # print("--------ssdasfas------------: ", abs_rel.size(),sq_rel.size(),rmse.size(),rmse_log.size())
        mask = ~mask
        abs_rel[mask] = 0
        sq_rel[mask] = 0
        rmse[mask] = 0
        rmse_log[mask] = 0
        # print("---------ab: ",torch.mean(abs_rel)*(384*640)/all)
    else:
        abs_rel = torch.abs(depth_pred_median  - depth_gt) / depth_gt
        sq_rel = (depth_pred_median  - depth_gt) ** 2 / depth_gt
        rmse = (depth_pred_median  - depth_gt) ** 2
        rmse_log = (torch.log(depth_pred_median ) - torch.log(depth_gt)) ** 2
        # print("--------ssdasfas------------: ", abs_rel.size(),sq_rel.size(),rmse.size(),rmse_log.size())
        mask = ~mask
        abs_rel[mask] = 0
        sq_rel[mask] = 0
