...
class VF_M2depth_ddp:
    ...
    def save_model(self, batch):
        ...
        save_folder = os.path.join(self.log_path, "models",
                                   "weights_{}".format(str(self.epoch) + 'e' + '_' + str(batch) + 'b'))
    ...
    def process_batch(self,inputs):
        ...
        #     np.save('debug_files/npy/depth_d0.npy', depth1)
        #     print("save depth over")
    ...
    #         m_depth_dict = {}
    #
    #         for k in self.models[load_models[0]].state_dict().keys():
    #             if 'module'in k:
    #                 m_depth_dict[k] = depth_dict[k[7:]]
    #             else:
    #                 m_depth_dict[k] = depth_dict[k]
    #         m_depth_dict = {k: v for k, v in m_depth_dict.items() if k in self.models[load_models[0]].state_dict()}
    #
    #         m_pose_dict = {}
    #         for k in self.models[load_models[1]].state_dict().keys():
    #             if 'module'in k:
    #                 m_pose_dict[k] = pose_dict[k[7:]]
    #             else:
    #                 m_pose_dict[k] = pose_dict[k]
    #         m_pose_dict = {k: v for k, v in m_pose_dict.items() if k in self.models[load_models[1]].state_dict()}
    #
    #
    #
    #         # depth_dict = {k[8:]:v  for k,v in depth_dict.items()}
    #         # pose_dict = {k[8:]:v for k,v in pose_dict.items()}
    #
    #         self.models[load_models[0]].load_state_dict(m_depth_dict)
    #         self.models[load_models[1]].load_state_dict(m_pose_dict)
    #         self.optimizer.load_state_dict(optim_file_path)
    ...
    def tensor_board_writer(self,inputs,outputs,losses,batch_idx):
        ...
        if (self.len_epoch * self.epoch + batch_idx)  % 10 == 0:
            if self.step % 1000 ==0:
                print("save board: ",self.step//1000)
            if (self.len_epoch * self.epoch + batch_idx) % 100 == 0:
                ...
                self.writer.add_image("image/origin_image_2", inputs[('color', 0, 0)].detach().cpu()[0][2],
                                      (self.len_epoch * self.epoch + batch_idx)  // 50)
                print("save board: ", (self.len_epoch * self.epoch + batch_idx) * (self.epoch + 1))
            self.writer.add_scalar('loss/reproj_loss', losses['reproj_loss'], (self.len_epoch * self.epoch + batch_idx) // 10)
            self.writer.add_scalar('loss/spatio_loss', losses['spatio_loss'], (self.len_epoch * self.epoch + batch_idx) // 10)
            self.writer.add_scalar('loss/spatio_tempo_loss', losses['spatio_tempo_loss'], (self.len_epoch * self.epoch + batch_idx) // 10)
            self.writer.add_scalar('loss/smooth', losses['smooth'], (self.len_epoch * self.epoch + batch_idx) // 10)
            self.writer.add_scalar('total_loss', losses['total_loss'], (self.len_epoch * self.epoch + batch_idx) // 10)
        else:
            pass
    ...
    def run_per_epoch(self):
        ...
        for batch_idx, inputs in enumerate(self.dataloaders['train']):
            ...
            if self.step % 200 == 0 and self.rank == self.rank_start:
                print("start_val-----")
