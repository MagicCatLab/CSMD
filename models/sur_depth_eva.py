...
class Sur_depth_ddp_eva:
    ...
    def init_losses(self, cfg, rank):
        print("self.spatio_temporal or self.spatio: ", self.spatio_temporal or self.spatio)
        if self.aug_depth:
    ...
    def process_batch(self,inputs):
        ...
        sur_depth = net_depth(inputs)
        # print("----------------------------now get models' layer name:------------------------ ")
        # for name, module in net_depth.named_modules():
        #     print(name)
        # print("----------------------------end get models' layer name:------------------------ ")
            # dot = make_dot(sur_depth)
            # dot.view(filename='model_structure', directory='./debug_files')
    ...
    def evaluate(self, save_root, save_del= False, save_pred=False):
        print("final2")
        av_per_eve = [(0,0)]
        ...
        avg_depth_single_cam_dict =  {f"cam{i}": {"metric": {'abs_rel': 0, 'sq_rel': 0, 'rms': 0, 'log_rms': 0, 'a1': 0, 'a2': 0, 'a3': 0}, "median":  {'abs_rel': 0, 'sq_rel': 0, 'rms': 0, 'log_rms': 0, 'a1': 0, 'a2': 0, 'a3': 0}} for i in range(6)}
        d_list = []
        ...
        for batch_idx, inputs in enumerate(eval_dataloader):
            ...
            with torch.no_grad():
                ...
                # print("inputs: ", inputs.keys())
                if save_pred:
                    self.save_depth_and_inter(save_root,inputs,outputs,save_del)
                # self.save_depth(outputs,inputs)
                if save_del:
                    depth_eval_metric, depth_eval_median, single_cam_dict, del_list = self.logger.compute_depth_losses_cam(inputs,
                                                                                                                 outputs,
                                                                                                                 save_del=save_del)
                    self.save_depth_del_and_inter(save_root,del_list)
                else:
                    depth_eval_metric, depth_eval_median, single_cam_dict = self.logger.compute_depth_losses_cam(inputs, outputs,save_del=save_del)

                # print("single_cam_dicr",single_cam_dict)
                ...
                for key in self.depth_metric_names:
                    ...
                    for cam_id in range(self.num_cams):
                        avg_depth_single_cam_dict["cam" + str(cam_id)]["metric"][key] += single_cam_dict["cam" + str(cam_id)]["metric"][key]
                        avg_depth_single_cam_dict["cam" + str(cam_id)]["median"][key] += single_cam_dict["cam" + str(cam_id)]["median"][key]
                        # print("n_cams",self.num_cams)
                # if batch_idx % 20 ==0:\
                #     print("------------------",batch_idx,"-----------------")
                #     print("depth_eval_metric: ",depth_eval_metric)
                #     print("depth_eval_median: ",depth_eval_median)
        ...
        for key in self.depth_metric_names:
            ...
            for cam_id in range(self.num_cams):
                avg_depth_single_cam_dict["cam" + str(cam_id)]["metric"][key] /= len(eval_dataloader)
                avg_depth_single_cam_dict["cam" + str(cam_id)]["median"][key] /= len(eval_dataloader)
        ...
        for cam_id in range(self.num_cams):
            print("metric-{}: ".format(cam_id),avg_depth_single_cam_dict["cam" + str(cam_id)]["metric"])
            print("median-{}: ".format(cam_id), avg_depth_single_cam_dict["cam" + str(cam_id)]["median"])

        self.write_dis_txt('dis_199.txt',d_list)

    def vis_rel_pixbdep(self):
        print("final2")
        av_per_eve = [(0,0)]
        ...
        print("length obj: ", len(eval_dataloader))
        i = 0
        depth_tensor = torch.zeros([1, 6, 1, 384, 640])
        ...
        for batch_idx, inputs in enumerate(eval_dataloader):
            ...
            i+=1
            # print(inputs['mask'].size())
            # depth.append(inputs['depth'])
            depth_tensor = depth_tensor + inputs['depth']
        depth_final = depth_tensor
        print("final av depth: ", depth_final.size() )
        depth_final = depth_final.detach().cpu()
        depth_final = depth_final.numpy()
        # np.save('./debug_files/npy/ddad_av_depth0.npy', depth_final)
        np.save('./debug_files/npy/nusc_av_depth0.npy', depth_final)
        print("over: and i is:{}".format(i))

    def save_depth_and_inter(self, save_root, inputs, outputs, save_del):
        depth_gt = inputs['depth']
        depth_gt = depth_gt.detach().cpu().numpy()

        images = inputs[('color_aug', 0, 0)].detach().cpu().numpy()
        # print("depth_truth shape is: ",depth_gt.shape)
        depth_pred = []
        for cam in range(self.num_cams):
            depth_pred_ele = outputs['cam', cam][('depth', 0)].detach().cpu().numpy()
            depth_pred.append(depth_pred_ele)
            # print("depth_pred shape is: ",depth_pred.shape)
        depth_pred = np.stack(depth_pred, axis=1)
        # print("depth_pred is: ",depth_pred.shape)

        new_folder = os.path.join(save_root, str(self.save_index))
        os.makedirs(new_folder, exist_ok=True)

        # np.save(os.path.join(new_folder, "depth_gt.npy"), depth_gt)
        np.save(os.path.join(new_folder, "depth_pred.npy"), depth_pred)
        np.save(os.path.join(new_folder, "imgs.npy"), images)
        print("save have finish: {} data".format(self.save_index))
        if save_del:
            pass
        else:
    ...
    def save_depth_del_and_inter(self, save_root,save_list):

        new_folder = os.path.join(save_root, str(self.save_index))
        os.makedirs(new_folder, exist_ok=True)

        # print(type(save_list[0]),save_list[0].detach().cpu().numpy().shape)
        # abs_rel_met, sq_rel_met, rmse_met, rmse_log_met, abs_rel_med, sq_rel_med, rmse_med, rmse_log_med
        np.save(os.path.join(new_folder, "abs_rel_met.npy"), save_list[0].detach().cpu().numpy())
        np.save(os.path.join(new_folder, "sq_rel_met.npy"), save_list[1].detach().cpu().numpy())
        np.save(os.path.join(new_folder, "rmse_met.npy"), save_list[2].detach().cpu().numpy())
        np.save(os.path.join(new_folder, "rmse_log_met.npy"), save_list[3].detach().cpu().numpy())
        np.save(os.path.join(new_folder, "abs_rel_med.npy"), save_list[4].detach().cpu().numpy())
        np.save(os.path.join(new_folder, "sq_rel_med.npy"), save_list[5].detach().cpu().numpy())
        np.save(os.path.join(new_folder, "rmse_med.npy"), save_list[6].detach().cpu().numpy())
        np.save(os.path.join(new_folder, "rmse_log_med.npy"), save_list[7].detach().cpu().numpy())
