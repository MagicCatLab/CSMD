...
class MultiCamLoss(SingleCamLoss):
    ...
    def spvs_losses(self,inputs, outputs, cam):
        gt_depth = inputs['depth'][:,cam,...]
        # depth_pred = inputs['depth'][:,cam,...]
        depth_pred = outputs[('depth', 0)]
        mask = torch.logical_and(gt_depth > 1.5, gt_depth < 200)

        pred_depth = depth_pred[mask]

        gt_depth = gt_depth[mask]

        loss =  F.l1_loss(pred_depth, gt_depth, size_average=True)
        return loss
    ...
    def forward(self, inputs, outputs, cam):        
        ...
        # print("self scale is： ", self.scales) [0]
        for scale in self.scales:
            ...
            # print("type reprojection_loss: ",type(reprojection_loss),reprojection_loss.size())
            ...
            gt_loss = self.spvs_losses(inputs, target_view, cam)
            ...
            if self.loss_gt:
                ...
                cam_loss += self.sys_coeff * gt_loss
            ##########################
            # for logger
            ##########################
            if scale == 0:
                loss_dict['reproj_loss'] = reprojection_loss.item()
                loss_dict['spatio_loss'] = self.spatio_coeff * spatio_loss.item()
                loss_dict['spatio_tempo_loss'] =  self.spatio_tempo_coeff * spatio_tempo_loss.item()
                loss_dict['smooth'] = smooth_loss.item()

                # here
                loss_dict['gt_loss'] =self.sys_coeff * gt_loss.item()
                if self.pose_model == 'fsm' and cam != 0:
