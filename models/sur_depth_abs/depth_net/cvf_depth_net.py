...
class Sur_VF_DepthNet(nn.Module):
    """
    Depth fusion module
    """

    def __init__(self, cfg, rank):
        super(Sur_VF_DepthNet, self).__init__()
    ...
    def forward(self, inputs):
        ...
        for i in range(len(feat_in)):
            ...
            # print("before cvt is: ",feat_in[i].view(-1, 6, C, H, W).size())
            feat_cvt_in.append(feat_in[i]+self.cross[i](feat_in[i].view(-1, 6, C, H, W)).view(B, C, H, W))
            # if i == len(feat_in)-1:
            #
            #     print(self.cross[i].attn_weights_dict.keys())
            #     for j in range(8):
            #         print("{}_iter attention isL: ".format(j),self.cross[i].attn_weights_dict['iter_{}'.format(j)].size())
