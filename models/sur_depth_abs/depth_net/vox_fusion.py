...
class VFNet(nn.Module):
    ...
    def forward(self, inputs, feats_agg):
        ...
        # print("--------------feat_agg size is: ",feats_agg.size() )
        voxel_feat = self.backproject_into_voxel(feats_agg, mask, K, extrinsics_inv)

        # print("--------------------voxel_feat:", type(voxel_feat), voxel_feat.size())
        ...
        if self.model == 'depth':
            ...
            # print("proj_feats: ", type(proj_feats), proj_feats[0].size(),proj_feats[1].size())
