...
from models.vox_depth_abs.losses.multi_cam_loss import MultiCamLoss,SingleCamLoss
import tqdm
from torchvision.utils import save_image

from models.vox_depth_abs.depth_net.cvf_depth_net import VIT_VF_DepthNet
from models.vox_depth_abs.pose_net.single_img_net.fusion_non_share.fusion_posenet import ImgFusedPoseNet
...
class Vox_vit_depth_eva:
    def __init__(self, cfg, rank, start_rank):
        # 完成模式基本参数初始化
        self.rank = rank
        self.read_config(cfg)
        self.rank_start = start_rank
        ...
        print("rank?: ",self.rank,self.rank_start)
        self.save_index = 0
    ...
    def prepare_model(self,cfg):
        ...
        self.models = {}
        self.models["depth_estimation_net"] = VIT_VF_DepthNet(cfg,self.rank)
    ...
    def evaluate(self, save_root, save_del= False, save_pred=False):
        ...
        avg_depth_single_cam_dict = {
            f"cam{i}": {"metric": {'abs_rel': 0, 'sq_rel': 0, 'rms': 0, 'log_rms': 0, 'a1': 0, 'a2': 0, 'a3': 0},
                        "median": {'abs_rel': 0, 'sq_rel': 0, 'rms': 0, 'log_rms': 0, 'a1': 0, 'a2': 0, 'a3': 0}} for i
            in range(6)}
        d_list = []
        ...
        for batch_idx, inputs in enumerate(eval_dataloader):
            ...
            with torch.no_grad():
                ...
                # self.save_depth(outputs,inputs)
                depth_eval_metric, depth_eval_median, single_cam_dict = self.logger.compute_depth_losses_cam(inputs, outputs)
        ...
        self.write_dis_txt('dis.txt',d_list)
