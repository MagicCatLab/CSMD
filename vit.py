...
class CVT(nn.Module):
    ...
    def forward(self, x):  # B, N, C, H, W
        B, N, C, H, W = x.size()
        # print("originaol x is : ", x.size())\
        # print("device", x.device)
        x = x.view(-1, C, H, W)
        x = self.sep_conv(x).view(B, N, C, H // self.downsample_ratio, W // self.downsample_ratio)
        print("sep_conv:P ",x.size())
        # 这一步骤用于进行下采样
        # print("sampled x is : ", x.size())

        for i in range(self.iter_num):
            self.pos_embed = self.postional_embed(x[:, 0, :, :, :]).unsqueeze(1).repeat(1, N, 1, 1, 1)  # B, N, C, H, W
            # print("pos_embed is: ", self.pos_embed)
...
img = torch.randn(1, 6 , 64 , 192 , 320 )
img = img.view(1,1,64,192,320 * 6)
preds = cvt(img) # (1, 1000)

img2 = torch.randn(1, 6 , 64 , 96 , 160 )
img2 = img2.view(1,1,64,96,160 * 6)
preds2 = cvt2(img2) # (1, 1000)

img3 = torch.randn(1, 6 , 128 , 48 , 80 )
img3 = img3.view(1,1,128,48,80 * 6)
pred3 = cvt3(img3) # (1, 1000)


print("final _pred: ", preds.size(), preds2.size(), pred3.size())