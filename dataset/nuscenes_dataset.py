...
class NuScenesdataset(Dataset):
    ...
    def get_tranformation_mat(self, pose):
        """
        This function transforms pose information in accordance with DDAD dataset format but wht?
        """
    ...
    def get_pose(self, sample):
        # 从sample中获取位姿的token
        # 从sample中取camera的sample_data token


        cam_data_token = sample['data']['CAM_FRONT']

        # 使用token加载sample_data
        cam_sample_data = self.dataset.get('sample_data', cam_data_token)

        ego_pose = self.dataset.get(
            'ego_pose', cam_sample_data['ego_pose_token'])
        ego_rotation = Quaternion(ego_pose['rotation']).inverse
        ego_translation = - np.array(ego_pose['translation'])[:, None]
        world_to_ego = np.vstack([
            np.hstack((ego_rotation.rotation_matrix,
    ...
    def __getitem__(self, idx):
        ...
        if self.split == 'train':
            prev_token = sample_nusc['prev']
            next_token = sample_nusc['next']
            prev_sample = self.dataset.get('sample', prev_token)
            next_sample = self.dataset.get('sample', next_token)
            curr_pose = self.get_pose(sample_nusc)
            prev_pose = self.get_pose(prev_sample)
            next_pose = self.get_pose(next_sample)

            # 即为prev变换为cur
            prev_mat = np.dot(np.linalg.inv(curr_pose), prev_pose)
            next_mat = np.dot(np.linalg.inv(next_pose), curr_pose)
        else:
            pass
        ...
        for cam in self.cameras:
            ...
            # print("cam_sample",cam_sample.keys())
            # print("prev is:",cam_sample['prev'])
            ...
            if self.with_pose:
                ...
                #
                # data.update({
                #     'extrinsics_inv':  np.linalg.inv(self.get_current('extrinsics', cam_sample))
                # })
                # 这里由于原本的外参矩阵的变换于一般的几何学中刚好相反，不符合直觉，故舍弃
                data.update({
                    'extrinsics':self.get_current('extrinsics', cam_sample)
                })
                ...
                if self.split == 'train':
                    data.update({'prev_mat': prev_mat})
                    data.update({'next_mat': next_mat})
                else:
                    pass
